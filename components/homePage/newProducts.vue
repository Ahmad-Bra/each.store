<template>
  <div class="newProducts" data-aos="fade-left" data-aos-duration="1000" data-aos-easing="linear">
    <v-container fluid>
      <h1 class="text-red font-wight-bold py-5"> {{ $t('New products') }}</h1>
      <v-row class="align-center ">
        ...
        <v-col v-else cols="12" sm="12" lg="7">
          <swiper style="padding: 40px 0;" :grab-cursor="true"
            :pagination="{ el: '.swiper-pagination', clickable: true }" :modules="modules" :breakpoints="breakpoints"
            space-between="10" :navigation="{ prevIcon: '.prev', nextIcon: '.next' }" :autoplay="{ delay: 3000 }"
            effect="coverflow">
            <swiper-slide v-for="item in newItmes" :key="item.id" cols="3">
              <v-card>
                <v-card-item>
                  <v-card-subtitle class="img-holder" style="position: relative;">
                    <img style="height: 300px; max-width: 100%; " :src="ShownItem[item.title]
                  ...
                  <v-card-text>
                    <p>
                      ({{ item.title }})
                      {{
                        item.description.length <= 40 ? item.description : item.description.slice(0, 20) }} ... </p>
                        <v-rating v-model="item.rating" density="compact" color="yellow-darken-3" half-increment
                          readonly>
                        </v-rating>
                        <p>
                          <del>${{ item.price }}</del> From<span class="font-weight-bold text-red">
                            ${{
                  ...
                  <v-card-actions class="action">
                    <v-btn-toggle class="mb-5 action" v-model="ShownItem[item.title]">
                      <v-btn class="img-btn" size="x-small action" v-for="(pic, i) in item.images" :value="pic"
                        :key="i">
                        <img :src="pic" style="
                            width: 40px;
                            height: 40px;
                  ...
                  <div class="d-flex align-center">
                    <v-btn style="font-weight: bold;" icon=" mdi-heart" variant="text" @click="addToFav(item)">
                    </v-btn>
