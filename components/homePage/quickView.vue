                ...
                <v-container class="" width="750" style="position: relative;">
                    <v-row>
                        <v-col cols="6" class="px-5">
                            ...
                            <div v-if="!loading">
                                <img style="height: 300px; width: 100%;" :src="tapImg ? tapImg : product.thumbnail"
                                    alt="">
                        ...
                        <v-col cols="6" class="px-5">
                            <v-skeleton-loader v-if="loading"
                                type="article, article, article, actions"></v-skeleton-loader>
                            <div class="content card" v-if="!loading">
                                <h1> ({{ product.title }}) Samble - {{ product.category }} For sale </h1>
                                <v-rating class="pt-5" v-model="product.rating" density="compact"
                                    ...
                                    <del>${{ product.price }}</del> From<span class="font-weight-bold text-red">
                                        ${{
                                            product.price - Math.ceil(product.price *
                                                product.discountPercentage / 100) }}</span>
                                </p>
                                ...
                                <p class="mt-5"> subTotale : <span class="font-weight-bold text-red">
                                        ${{ Math.ceil((product.price - Math.ceil(product.price *
                                            product.discountPercentage / 100)) * quantity) }}</span> </p>
                                <v-btn :loading="btnLoading" @click="addToCart(product)" class="mt-10"
                                    style="color: white; background-color: black; font-weight: bold; width: 100%; height: 40px;"
                                    rounded> {{ $t('Add To Cart') }}</v-btn>
                            </div>
                        </v-col>
                    </v-row>
                </v-container>
