			...
			<v-row style="">
				<v-col cols="12" md="9">
					...
					<div class="d-flex align-center justify-space-between my-5">
						<p v-if="10000 - calcSubtotal < 0">Your order over free shipping </p>
						<p v-if="10000 - calcSubtotal > 0"> Only ${{ 10000 - calcSubtotal }} away from free
							shepping
						</p>
						<v-btn v-if="cartItem.length" @click="clearCart()" class="text-white bg-error">
							<v-icon>
								mdi-delete
							</v-icon>
							Clear Cart
				...
				<v-col class="mb-5" cols="12" md="3" v-for="item in cartItem" :key="item.id">
					...
					<p class="my-3">get shipping estimate : </p>
					...
					<div class="d-flex flex-column mx-auto">
						<v-btn density="comfortable" @click="navigateTo('/checkoutPage')"
							style="height: 50px; max-width: 300px; background-color: #ccc; color: black; margin: 0; "
							rounded>
							proceed to
							CheckOut</v-btn>
