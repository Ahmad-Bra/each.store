<template>
  ...
  <NuxtLayout />
  <div class="pro-category text-center" data-aos="zoom-out-up" data-aos-duration="10000" data-aos-easing="linear">
    <v-container fluid>
            ...
            <v-card elevation="0">
              <v-card-item>
                ...
                <v-card-text>
                  <p>
                    ({{ categor.title }})
                    {{ categor.description.length <= 40 ? categor.description : categor.description.slice(0, 40) }} ...
                      </p>
                      <v-rating v-model="categor.rating" density="compact" color="yellow-darken-3" half-increment
                        readonly>
                      </v-rating>
                      <p>
                        <del>${{ categor.price }}</del> From<span class="font-weight-bold text-red">
                          ${{
                ...
                <v-card-actions class="action">
                  <v-btn-toggle class="mb-5 action" v-model="ShownItem[categor.title]" mandatory>
                    <v-btn size="x-small action" v-for=" pic, i in categor.images " :value="pic" :key="i">
